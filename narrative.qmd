...
- <mark>Some measurements are in Standard and some in Imperial with units in the same file include both °F and °C, as well as measurements in meters, feet, and inches.</mark>  In fact, there is duplication of data columns in different units (guess what kind of correlation they might have...)
...
In the following examples, we'll be using the rice data above.  For each verb, I'm going to use the native pipe operator (`|>`) to send the data into the example functions and then assign the result to a dummy `data.frame` named `df`.  The arguments passed to each of the verbs are where the magic happens.


### The Output

The key to these activities is that every one of these functions takes a `data.frame` as input, does its operations on it, then returns a `data.frame` object as output.  The `data.frame` is the core data container for all of these actions.
...
Let's say that you wanted to reorder the columns as `RecordID`, `ODO_mgl` and `PH` as the first three columns and leave everything else as is.  There is this cool function `everything()` that helps out. 
...
Finally, `select()` can rename columns as it grabs them, using the form `NewName = OldName`.  This is handy when the raw data use terse or inconsistent column names and you want something readable to carry through into tables and figures.

```{r}
rice |>
  select( Time = DateTime,
          `Air Temp` = AirTempF,
          `Water Temp` = H2O_TempC ) -> df
names( df )
```

If you want to rename a column but keep every other column as-is, combine this with `everything()`.

```{r}
rice |>
  select( Time = DateTime, everything() ) -> df
names( df )[1:4]
```
...
The function `filter()` works to select records (rows) based upon some criteria.  <mark>So for example, if I am interested in just records when the airtemp was freezing (and the raw data are in °F).</mark>  The range of values in the original data was:
...
Each argument you give to the function performs one or more operations on the data and returns a brand new `data.frame` object with only the values specified.   
...
You are not limited to the mean.  Any function that takes a vector of numbers and returns a single value can be used here, and you can ask for several summaries of the same column at once.  Here I describe air temperature by its minimum, mean, and maximum.

```{r}
rice |>
  summarize( Minimum = min( AirTempF ),
             Mean = mean( AirTempF ),
             Maximum = max( AirTempF ) )
```
...
So far every result has come back as raw `R` console output.  That is fine while you are building a workflow, but a document that someone is meant to *read* needs proper tables --- captioned, numbered, and referred to *by number* in the running text, the way a table appears in a journal article.

### From a data frame to a table

The `kable()` function in the `knitr` package turns a `data.frame` into a formatted table.  You pipe the data straight into it, usually as the last step of a workflow.
...
rice |>
  ...
  group_by( Month ) |>
  summarize( `Air Temp` = mean( AirTempF ),
             `Water Temp` = mean( H2O_TempC, na.rm = TRUE ) ) -> monthlyTemps

monthlyTemps |>
  kable( digits = 1 )
```

The `kableExtra` package adds styling on top of that --- `kable_paper()`, `kable_styling()`, `add_header_above()`, and many more.  The [kableExtra vignette](https://haozhu233.github.io/kableExtra/awesome_table_in_html.html) walks through the options, and the slides for this module show several of them side by side.

### Cross-Referencing Tables in Text

A table is only useful if the reader knows which paragraph is talking about it.  Quarto does the numbering for you, but you have to give it two pieces of information *in the chunk that makes the table*:

- a **label** that begins with `tbl-`, set with the `#| label:` chunk option.  The `tbl-` prefix is what tells Quarto that the chunk produces a table.
- a **caption**, set with the `#| tbl-cap:` chunk option.

```{r}
#| label: tbl-monthly
#| tbl-cap: "Mean monthly air and water temperature at the Rice Rivers Center."
...
With the label in place, you refer to the table anywhere in your prose by writing `@tbl-monthly`.  Quarto replaces it with the table's number --- and keeps that number correct even if you add or reorder tables later.  For example, this sentence was written with `@tbl-monthly` standing in for the number:

> Mean monthly water temperature rose steadily from January through March (@tbl-monthly).

The same system works for figures (give the chunk a `fig-` label) and for equations, sections, and more.  It is also the reason you should never write "see the table below": position and hand-typed numbers both break the moment the document changes.
